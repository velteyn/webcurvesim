...
public class ExchangeFixAgent extends MessageCracker implements Application {
	...
	private void processCancelReject(Message message, SessionID sessionId) throws FieldNotFound
	{
        ...
        origOrder.setStatus(ClientOrder.STATUS.REJECTED);
        String info = "";
	...
	{
		// TODO: validate its fix message 4.2
        String clientOrderID = message.getField(new ClOrdID()).getValue();
        ClientOrder order = orders.get(clientOrderID);
        if (null == order)
        {
        ...
        {
            order.setStatus(ClientOrder.STATUS.REJECTED);
            orders.remove(clientOrderID);
        } 
        ...
        {
            order.setStatus(ClientOrder.STATUS.DONE);
        } 
        		  ...
        		  ordStatus.valueEquals(OrdStatus.FILLED) )
        {
	        ...
	        if (ordStatus.valueEquals(OrdStatus.PARTIALLY_FILLED))
	        	order.setStatus(ClientOrder.STATUS.LIVE);
	...
	public void subscribeMarketData(String symbol, ExchangeEventListener<MarketDepth> depthListener, ExchangeEventListener<MarketTrade> tradeListener)
	{
		quickfix.fix42.MarketDataRequest mdr;
		try {
			String mdRequestId = getNextMarketDataRequestId();
